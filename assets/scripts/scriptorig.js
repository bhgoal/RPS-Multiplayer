...
function startMatch() {
  console.log("begin startMatch function");
  if ((localPlayer === "player1") || (localPlayer === "player2")) {
    ...
    $("#matchPositionDisplay").text("Spectating");
    $("#matchMessageDisplay").text("Game in progress");
  }
}    
...
database.ref("/moves").on("value", function(snapshot) {
  ...
  if (snapshot.child("player1").exists() && snapshot.child("player2").exists()) {
    ...
    if (winner === player1) {
      ...
      $("#matchMoveStatus").append("<br>Tie game");
      player1.ties++;
      player2.ties++;
    }
...
$("#submit-chat").on("click", function(event) {
  ...
  event.stopPropagation();
  // Get the input values
  chatInput = (localName + ": " + $("#chat-form").val());
  database.ref("/chat").push(chatInput);
  $("#chatEntry")[0].reset();
});

database.ref("/chat").on("child_added", function(snapshot) {
    console.log(snapshot.val());
    $("#chatMessages").append(snapshot.val() + "<br>");
});